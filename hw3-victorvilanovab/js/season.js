...
class Season {
  constructor(containerElement, TV_SHOWS, i) {
    ...
    for (let temp=0; temp< TV_SHOWS[i]["seasons"].length; temp++){
      ...
      label.setAttribute('for',"tick_all_s"+i+"t"+temp);
      const eye_img = document.createElement('img');
      eye_img.src = '../images/eye-gray.png';
      eye_img.setAttribute('id',"img_tick_all_s"+i+"t"+temp);
      ...
      for(let ep=0; ep < TV_SHOWS[i]["seasons"][temp].length; ep++){
        const container_section_temporada = document.querySelector("#s"+i+"t"+Number(temp+1))
